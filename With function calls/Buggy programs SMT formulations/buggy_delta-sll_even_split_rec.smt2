...
;;ss := x.next
;;x.next := ff_temp
;***;Below is the effect of reassigning pointers in the wrong order:
;***;x.next := ff_temp
;***;x.next.next := ss_temp
;***;(assert (= n2 (store (store n (select n x) ss_temp) x ff_temp)))
(assert (= n2 (store (store n x ff_temp) (select (store n x ff_temp) (select (store n x ff_temp) x)) ss_temp)))
;;ff := x
