...
;;y.next := nil
;;x.next := y
(assert (= n2 (store (store n y nil) x y)))
;;return
...
(and
(ls2 x2 nil)
(= (mkeys2 x2 nil) (store (mkeys x2 nil) kk (+ (select (mkeys x2 nil) kk) 1)))
)))
;;frame-reasoning: heaplet of function call is hls(x2, nil)
(assert (=> (= (intersect (hls x x2) (hls x2 nil)) empLoc) 
(and 
(iff (ls x  x2) (ls2 x x2))
(= (mkeys x x2) (mkeys2 x x2))
)))
...
(assert (t_propagate_for_vars nil))
(assert (base x2))
(assert (n_propagate x x2))
(assert (t_propagate_for_vars x2))
