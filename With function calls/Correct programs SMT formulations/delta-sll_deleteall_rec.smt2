...
(assert (closed-sys x2) )
;;if x.key == kk then x2 := x.next
(assert (=> (= (key x) kk) (= x2 (select n x))))
;;if x.key =/= kk then x2 := x
(assert (=> (not (= (key x) kk)) (= x2 x)))

;;--delta is x
(assert (= inDelta (store empLoc x true)))
...
(assert (and (not (= x nil)) (not (= (key x) kk))))
;;declare temp
...
(and
(ls2 temp nil)
(= (select (mkeys2 temp nil) arb) (select (mkeys temp nil) arb))
(= (select (mkeys2 temp nil) kk) 0)
)))
;;frame-reasoning: heaplet of function call is hls(temp, nil)
(assert (=> (= (intersect (hls x temp) (hls temp nil)) empLoc) 
(and 
(iff (ls x  temp) (ls2 x temp))
(= (mkeys x temp) (mkeys2 x temp))
)))
;;frame-reasoning: observe that the pointers on the bounded part have not changed 
;;if not in the heaplet of the function call
(assert (=> (not (select (hls temp nil) x))
