...
(assert (inKeys (key x)))
(assert (closed-sys x))
(declare-const ff Loc)
(assert (inKeys (key ff)))
(assert (closed-sys ff))
(declare-const ss Loc)
(assert (inKeys (key ss)))
(assert (closed-sys ss))
(declare-const oldhls (Set Loc))
(declare-const oldmkeys (MultiSet Int))

(push)
;;assume x == nil || x.next == nil
(assert (or (= x nil) (= (select n x) nil)))
;;ff := x
(assert (= ff x))
;;ss := nil
(assert (= ss nil))
...
(ls ff nil)
(ls ss nil)
(= oldhls (union (hls ff nil) (hls ss nil)))
(= oldmkeys (mult-union (mkeys ff nil) (mkeys ss nil)))
(=> (not (= (lslen ff nil) (lslen ss nil))) (= (lslen ff nil) (+ (lslen ss nil) 1)))
)))
(check-sat)
(get-model)
(pop)
...
(push)
;;assume x =/= nil && x.next =/= nil
(assert (and (not (= x nil)) (not (= (select n x) nil))))
;;declare temp
(declare-const temp Loc)
(assert (inKeys (key temp))) 
;;temp := x.next.next
(assert (= temp (select n (select n x))))


;;declare ff_temp
(declare-const ff_temp Loc)
(assert (closed-sys ff_temp))
(assert (inKeys (key ff_temp)))
;;declare ss_temp
(declare-const ss_temp Loc)
(assert (closed-sys ss_temp))
(assert (inKeys (key ss_temp)))
;;recursively call even_split_rec(temp, ff_temp, ss_temp)
(declare-const oldhls_rec (Set Loc))
(assert (= oldhls_rec (hls temp nil)))
(declare-const oldmkeys_rec (MultiSet Int))
(assert (= oldmkeys_rec (mkeys temp nil)))
;;assert postcondition of function call if precondition is true
(assert (=> 
(and 
(ls temp nil)
(= oldhls_rec (hls temp nil))
(= oldmkeys_rec (mkeys temp nil))
);;antecedent
(and 
(ls2 ff_temp nil)
(ls2 ss_temp nil)
(= oldhls_rec (union (hls2 ff_temp nil) (hls2 ss_temp nil)))
(= oldmkeys_rec (mult-union (mkeys2 ff_temp nil) (mkeys2 ss_temp nil)))
(=> (not (= (ls2len ff_temp nil) (ls2len ss_temp nil))) 
	(= (ls2len ff_temp nil) (+ (ls2len ss_temp nil) 1)))
);;consequent
))
...
;;ss := x.next
;;x.next := ff_temp
(assert (= n2 (store (store n (select n x) ss_temp) x ff_temp)))
;;ff := x
(assert (= ss (select n x)))
(assert (= ff x))
...
(assert (n_propagate (select n x) temp))
...
(ls2 ff nil)
(ls2 ss nil)
(= oldhls (union (hls2 ff nil) (hls2 ss nil)))
(= oldmkeys (mult-union (mkeys2 ff nil) (mkeys2 ss nil)))
(=> (not (= (ls2len ff nil) (ls2len ss nil))) (= (ls2len ff nil) (+ (ls2len ss nil) 1)))
)))
(assert (base2 nil))
(assert (n2_propagate x nil))
(assert (n2_propagate (select n x) nil))
(assert (base2 temp))
(assert (n2_propagate x temp))
(assert (n2_propagate (select n x) temp))
(check-sat)
(get-model)
(pop)
