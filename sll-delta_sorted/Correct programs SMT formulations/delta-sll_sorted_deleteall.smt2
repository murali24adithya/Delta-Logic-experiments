...
(assert (inKeys kk))
(declare-const arb Int)
(assert (inKeys arb))
;;arb =/= kk
(assert (not (= arb kk)))
(declare-const constval Int)
;;declare temp
...
;@precondition: list(x) && sorted(x) && mkeys(x).arb == constval
(assert (and 
(ls x nil)
(sorted x nil)
(= (select (mkeys x nil) arb) constval)
))
(assert (base nil))
(assert (t_propagate_for_vars nil))

;negate@x = temp && list(x) && sorted(x) && mkeys(x).kk = mkeys(temp).kk
;;&& mkeys(x).arb == constval
(assert (not (and 
(= x temp)
(ls x nil)
(sorted x nil)
(= (select (mkeys x nil) kk) (select (mkeys temp nil) kk))
(= (select (mkeys x nil) arb) constval)
)))
(check-sat)
(get-model)
...
;@x = temp && list(x) && sorted(x) && mkeys(x).kk = mkeys(temp).kk
;;&& mkeys(x).arb == constval
(assert (and 
(= x temp)
(ls x nil)
...
(ls x2 nil)
(sorted x2 nil)
(= (select (mkeys x2 nil) kk) (select (mkeys temp2 nil) kk))
(= (select (mkeys x2 nil) arb) constval)
)))

(check-sat)
(get-model)
(pop);;end-of-while temp.key =/= kk

(push)
(assert (not (= (key temp) kk)))

;@ls(x, temp) && list(x) && sorted(x)
;; && mkeys(x).kk = mkeys(temp).kk && temp.key =/= kk
;;&& mkeys(x).arb == constval
(assert (and 
(ls x temp)
(ls x nil)
...
(assert (not (= temp nil)) )
(assert (not (= (select n temp) nil)) )

(push)
;;assume temp.next.key = kk 
(assert (= (key (select n temp)) kk) )
;;temp.next = temp.next.next
(assert (= n2 (store n  temp (select n (select n temp))) ) )
...
(assert (t-structure-for-hls (select n temp)))
(assert (closed-sys (select n temp)) )
(assert (closed-sys (select n (select n temp))) )

;;for precondition
(assert (n_propagate temp nil))
(assert (n_propagate (select n temp) nil))
(assert (n_propagate temp temp))
(assert (n_propagate (select n temp) temp))

;negate@ls(x, temp) && list(x) && sorted(x)
;; && mkeys(x).kk = mkeys(temp).kk && temp.key =/= kk
;;&& mkeys(x).arb == constval
(assert (not (and 
(ls2 x temp)
(ls2 x nil)
(sorted2 x nil)
(= (select (mkeys2 x nil) kk) (select (mkeys2 temp nil) kk))
(not (= (key temp) kk))			
(= (select (mkeys2 x nil) arb) constval)
)))
(assert (base2 nil))
(assert (n2_propagate temp nil))
(assert (n2_propagate (select n temp) nil))
(assert (t_propagate2_for_vars nil))

(assert (base2 temp))
(assert (n2_propagate temp temp))
(assert (n2_propagate (select n temp) temp))
(assert (t_propagate2_for_vars temp))
(check-sat)
(get-model)
(pop);;end of temp.next.key == kk branch

(push)
;;assume temp.next.key =/= kk 
(assert (not (= (key (select n temp)) kk)) )
;;temp = temp.next
(declare-const temp2 Loc)
(assert (inKeys (key temp2)))
(assert (= temp2 (select n temp)) )
...
(ls x nil)
(sorted x nil)
(= (select (mkeys x nil) kk) (select (mkeys temp2 nil) kk))
(not (= (key temp2) kk))			
(= (select (mkeys x nil) arb) constval)
)))
(assert (base temp2) )
...
;negate@list(x) && sorted(x) && mkeys(x).kk = 0
;;&& mkeys(x).arb == constval
(assert (not (and 
(ls x nil) 
(sorted x nil)
(= (select (mkeys x nil) kk) 0)
(= (select (mkeys x nil) arb) constval)
)))
(check-sat)
(get-model)
