...
;negate@ls(x) && ls(y) && sorted(x) && revsorted(y)
...
(sorted x nil)
(revsorted y nil)
(=> (and (not (= x nil)) (not (= y nil))) (>= (minls x nil) (maxls y nil)))
...
;@ls(x,nil) && ls(y,nil) && sorted(x) && revsorted(y)
;;&& minls(x) >= maxls(y)
;;&& hls(x,nil) \cap hls(y,nil) = \empty 
;;&& oldhls = hls(x) \union hls(y)
;;&& oldmkeys = mkeys(x) \mult-union mkeys(y) 
...
;negate@ls2(x2,nil) && ls2(y2,nil) && sorted2(x2) && revsorted2(y2)
;;&& minls2(x2) >= maxls2(y2)
...
(sorted2 x2 nil)
(revsorted2 y2 nil)
(=> (and (not (= x2 nil)) (not (= y2 nil))) (>= (minls2 x2 nil) (maxls2 y2 nil)))
...
;negate@list(x) && revsorted(x) && hls(x) = oldhls && mkeys(x) = oldmkeys
(assert (not (and 
(ls x2 nil)
(revsorted x2 nil) 
(= (hls x2 nil) oldhls)
