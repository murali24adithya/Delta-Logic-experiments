...
;;minimal correct program: observe that vanilla frame reasoning does not yield a proof
;@pre: ls(x, nil) && ls(w, nil) && y \notin hls(w, nil) 
;;{y.next := w}
;@post: ls(x, nil)
;;-----------------------------------------------------------
...
;;y.next := w
(assert (= n2 (store n y w)))

;;--delta is y
(assert (= inDelta (store empLoc y true)))
