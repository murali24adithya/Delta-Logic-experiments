...
;negate@ list(x) && mkeys2(x) = mkeys(x) \mult-union {kk}
(assert (not (and 
(ls2 x2 nil)
(= (mkeys2 x2 nil) (store (mkeys x nil) kk (+ (select (mkeys x nil) kk) 1)))
)))
(assert (base2 nil))
...
;negate@ls(x, temp) && temp =/= nil
(assert (not (and (ls x temp) (not (= temp nil)))))
(assert (base temp))
...
;@ls(x, temp) && temp =/= nil
(assert (and (ls x temp) (not (= temp nil))))
(assert (base temp))
(assert (n_propagate temp temp))
(assert (t_propagate_for_vars temp))

;negate@ls(x, temp) && temp =/= nil
(assert (not (and (ls x temp2) (not (= temp2 nil)))))
(assert (base temp2))
...
;;temp.next = y
;;y.next = nil
(assert (= n2 (store (store n temp y) y nil)))
...
;negate@ list(x) && mkeys2(x) = mkeys(x) \mult-union {kk}
(assert (not (and (ls2 x nil)
(= (mkeys2 x nil) (store (mkeys x nil) kk (+ (select (mkeys x nil) kk) 1))))))
(assert (base2 nil))
