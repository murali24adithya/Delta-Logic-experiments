...
;;cycle_detection(x): detects if there is a cyle in x
(declare-const x Loc)
(assert (inKeys (key x)))
(assert (closed-sys x))
(declare-const found Bool)
(declare-const tort Loc)
(assert (inKeys (key tort)))
(assert (closed-sys tort))
(declare-const hare Loc)
(assert (inKeys (key hare)))
(assert (closed-sys hare))

(push)
;;if x == nil or x.next == nil or x.next.next == nil
(assert (or 
(= x nil)
(= (select n x) nil)
(= (select n (select n x)) nil)
))
;;found = false
(assert (not found))
...
(assert (closed-sys (select n (select n x))))
(assert (t-structure-for-hls x))
(assert (t-structure-for-hls (select n x)))

;@no precondition
;negate@postcondition: found => ls (x, tort) && circ_ls(tort.next, hare)
;;&&not found => ls(x, nil)
(assert (not (and 
(=> found (and (ls x tort) (circ_ls (select n tort) hare)))
(=> (not found) (ls x nil))
)))
(assert (base nil))
(assert (n_propagate x nil))
(assert (n_propagate (select n x) nil))
...
(push)
;;x =/= nil and x.next =/= nil and x.next.next =/= nil
(assert (and  
(not (= x nil))
(not (= (select n x) nil))
(not (= (select n (select n x)) nil))
))
;;tort = x.next
(assert (= tort (select n x)))
;;hare = x.next.next
(assert (= hare (select n (select n x))))
...
;@no precondition
;negate@ls(x, tort) && ls(tort.next, hare)
(assert (not (and
(ls x tort)
(ls (select n tort) hare)
)))
(assert (base tort))
(assert (n_propagate x tort))
(assert (n_propagate (select n x) tort))
(assert (t_propagate_for_vars tort))
(assert (base hare))
(assert (n_propagate x hare))
(assert (n_propagate (select n x) hare))
(assert (t_propagate_for_vars hare))
(check-sat)
(get-model)
(pop)
...
(assert (closed-sys (select n hare)))
(assert (t-structure-for-hls hare))
(assert (closed-sys (select n tort)))
(assert (t-structure-for-hls tort))
...
(assert (base tort))
(assert (n_propagate hare tort))
(assert (n_propagate tort tort))
(assert (t_propagate_for_vars tort))
(assert (base hare))
(assert (n_propagate hare hare))
(assert (n_propagate tort hare))
(assert (t_propagate_for_vars hare))
...
(assert (base nil))
(assert (n_propagate hare nil))
(assert (n_propagate tort nil))
(assert (t_propagate_for_vars nil))
(check-sat)
(get-model)
(pop)
...
(push)
;;if hare.next =/= nil
(assert (not (= (select n hare) nil)))
;;hare = hare.next.next
(declare-const hare2 Loc)
(assert (inKeys (key hare2)))
(assert (= hare2 (select n (select n hare))))
;;tort = tort.next
(declare-const tort2 Loc)
(assert (inKeys (key tort2)))
(assert (= tort2 (select n tort)))

;;--delta is tort, hare, hare.next, tort2 (since mentioned in postcondition)
(assert (= inDelta
(store (store (store (store empLoc
tort true)
hare true)
(select n hare) true)
tort2 true)
...
(assert (closed-sys (select n hare)))
(assert (t-structure-for-hls hare))
(assert (closed-sys (select n (select n hare))))
(assert (t-structure-for-hls (select n hare)))
(assert (closed-sys (select n tort2)))
(assert (t-structure-for-hls tort2))
...
(assert (n_propagate tort tort))
(assert (n_propagate hare tort))
(assert (n_propagate (select n hare) tort))
(assert (n_propagate tort2 tort))
(assert (t_propagate_for_vars tort))
(assert (base hare))
(assert (n_propagate tort hare))
(assert (n_propagate hare hare))
(assert (n_propagate (select n hare) hare))
(assert (n_propagate tort2 hare))
(assert (t_propagate_for_vars hare))

;negate@ls(x, tort) && ls(tort.next, hare)
(assert (not (and
(ls x tort2)
(ls (select n tort2) hare2)
)))
(assert (base tort2))
(assert (n_propagate tort tort2))
(assert (n_propagate hare tort2))
(assert (n_propagate (select n hare) tort2))
(assert (n_propagate tort2 tort2))
(assert (t_propagate_for_vars tort2))
(assert (base hare2))
(assert (n_propagate tort hare2))
(assert (n_propagate hare hare2))
(assert (n_propagate (select n hare) hare2))
(assert (n_propagate tort2 hare2))
(assert (t_propagate_for_vars hare2))
(check-sat)
(get-model)
(pop)
