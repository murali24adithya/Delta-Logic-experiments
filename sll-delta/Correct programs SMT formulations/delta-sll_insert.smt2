...
;@precondition: list(x) && oldmkeys = mkeys(x, nil)
(assert (and 
(ls x nil)
...
;negate@ list2(x) && mkeys2(x) = oldmkeys \mult-union {kk}
(assert (not (and 
(ls2 x2 nil)
(= (mkeys2 x2 nil) 
   (store oldmkeys 
...
(assert (and (not (= (select n temp) nil)) 
        (<= (key (select n temp)) kk)))
;;temp = temp.next
