...
(ls x nil)
(ls x temp)
(iff found (not (= (select (mkeys x temp) kk) 0)))
(= (mkeys x nil) oldmkeys)
(= (hls x nil) oldhls)
)))
(assert (base temp) )
...
(assert (iff (or (= (key temp) kk) found) found2))
;;temp = temp.next
...
;@list(x) && ls(x, temp) && found <=> mkeys(x, temp).kk > 0
;; && mkeys(x, nil) = oldmkeys && hls(x, nil) = oldhls
(assert (and 
(ls x nil)
(ls x temp)
...
(ls x nil)
(ls x temp2)
(iff found2 (not (= (select (mkeys x temp2) kk) 0)))
(= (mkeys x nil) oldmkeys)
(= (hls x nil) oldhls)
)))
(assert (base temp2) )
...
;@post: list(x) && mkeys(x) = mkeys2(x)??? && (found <=> mkeys(x).kk > 0) 
;; && mkeys(x, nil) = oldmkeys && hls(x, nil) = oldhls
(assert (not(and 
(ls x nil)
(iff found (not (= (select (mkeys x nil) kk) 0)))
(= (mkeys x nil) oldmkeys)
(= (hls x nil) oldhls)
)))
(check-sat)
