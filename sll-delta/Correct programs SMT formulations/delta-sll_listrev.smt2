...
;@precondition: list(x) && oldhls = hls(x) && oldmkeys = mkeys(x)
(assert (and 
(ls x nil)
(= oldhls (hls x nil))
...
;negate@ls(x,nil) /\ ls(y,nil) /\ hls(x,nil) \cap hls(y,nil) = \empty
;;&& oldhls = hls(x) \union hls(y)
;;&& oldmkeys = mkeys(x) \mult-union mkeys(y) 
(assert (not (and 
(ls x nil) 
(ls y nil)
(= (intersect (hls x nil) (hls y nil)) empLoc)
(= (union (hls x nil) (hls y nil)) oldhls) 
(= (mult-union (mkeys x nil) (mkeys y nil)) oldmkeys) 
)))
(check-sat)
(get-model)
(pop)
...
;@ls*(x,nil) /\ ls*(y,nil) /\ hls*(x,nil) \cap hls*(y,nil) = \empty 
;;&& oldhls = hls(x) \union hls(y)
;;&& oldmkeys = mkeys(x) \mult-union mkeys(y) 
(assert (and 
(ls x nil) 
...
;negate@ls2(x2,nil) /\ ls2(y2,nil) /\ hls2(x2,nil) \cap hls2(y2,nil) = \empty 
;;&& oldhls = hls2(x2) \union hls2(y2)
;;&& oldmkeys = mkeys2(x2) \mult-union mkeys2(y2) 
(assert (not (and 
(ls2 x2 nil) 
(ls2 y2 nil)
(= (intersect (hls2 x2 nil) (hls2 y2 nil)) empLoc)
(= (union (hls2 x2 nil) (hls2 y2 nil)) oldhls) 
(= (mult-union (mkeys2 x2 nil) (mkeys2 y2 nil)) oldmkeys) 
)))
(assert (base2 nil))
...
;negate@list(x) && hls(x) = oldhls && mkeys(x) = oldmkeys
(assert (not (and 
(ls x2 nil) 
(= (hls x2 nil) oldhls)
(= (mkeys x2 nil) oldmkeys)
)))
(check-sat)
(get-model)
(pop)
